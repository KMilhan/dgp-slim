...
DATASET_SPLIT_NAME_TO_KEY = OrderedDict(
    {k.lower(): v for k, v in dataset_pb2.DatasetSplit.items()}
)
DATASET_SPLIT_KEY_TO_NAME = OrderedDict(
    {v: k for k, v in DATASET_SPLIT_NAME_TO_KEY.items()}
)
...
ANNOTATION_KEY_TO_TYPE_ID = OrderedDict(
    {k.lower(): v for k, v in annotations_pb2.AnnotationType.items()}
)
ANNOTATION_TYPE_ID_TO_KEY = OrderedDict(
    {v: k for k, v in ANNOTATION_KEY_TO_TYPE_ID.items()}
)
...
FEATURE_KEY_TO_TYPE_ID = OrderedDict(
    {k.lower(): v for k, v in features_pb2.FeatureType.items()}
)
