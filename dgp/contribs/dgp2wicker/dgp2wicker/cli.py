...
@click.option(
    ...
    required=True,
    default=None,
    help="Name of dataset in Wicker",
)
@click.option(
    "--wicker-dataset-version", required=True, help="Version of dataset in Wicker"
)
...
@click.option(
    "--only-annotated-datums", is_flag=True, help="Apply only annotated datums"
)
@click.option(
    "--max-num-scenes",
    required=False,
    default=None,
    help="The maximum number of scenes to process",
)
@click.option(
    "--max-len",
    required=False,
    default=1000,
    help="The maximum number of samples per scene",
)
@click.option(
    "--chunk-size", required=False, default=1000, help="The number of samples per chunk"
)
@click.option(
    "--skip-camera-cuboids",
    is_flag=True,
    help="If True, skip cuboids for non lidar datums",
)
@click.option(
    "--num-partitions", required=False, default=None, help="Number of scene partitions"
)
@click.option(
    "--num-repartitions",
    required=False,
    default=None,
    help="Number of sample partitions",
)
@click.option(
    "--is-pd",
    is_flag=True,
    help="If true, process the dataset with ParallelDomainScene",
)
@click.option(
    "--data-uri", required=False, default=None, help="Alternate location for scene data"
)
@click.option(
    "--add-lidar-points", is_flag=True, help="Add lidar point count to lidar cuboids"
)
@click.option(
    "--half-size-images", is_flag=True, help="Resize image datums to half size"
)
@click.option(
    "--alternate-scene-uri",
    required=False,
    default=None,
    help="Alternate scene locaiton to sync",
)
def ingest(
    scene_dataset_json,
    wicker_dataset_name,
    ...
    requested_annotations = (
        [x.strip() for x in requested_annotations.split(",")]
