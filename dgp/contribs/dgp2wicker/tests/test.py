...
TEST_WICKER_DATASET_JSON = os.path.join(
    TEST_DATA_DIR, "test_scene", "scene_dataset_v1.0.json"
)
...
def s3_is_configured():
    ...
    wicker_config_path = os.getenv(
        "WICKER_CONFIG_PATH", os.path.expanduser("~/wickerconfig.json")
    )
...
class TestDDGP2Wicker(unittest.TestCase):
    ...
    def test_schema(self):
        ...
        wicker_types = wicker_types_from_sample(
            sample, ontology_table, skip_camera_cuboids=True
        )
        expected_keys = [
            "CAMERA_01____timestamp",
            "CAMERA_01____rgb",
            "CAMERA_01____intrinsics",
            "CAMERA_01____distortion",
            "CAMERA_01____extrinsics",
            "CAMERA_01____pose",
            "CAMERA_01____bounding_box_2d",
            "CAMERA_01____datum_type",
            "LIDAR____timestamp",
            "LIDAR____extrinsics",
            "LIDAR____pose",
            "LIDAR____point_cloud",
            "LIDAR____extra_channels",
            "LIDAR____bounding_box_3d",
            "LIDAR____datum_type",
            "ontology____bounding_box_2d",
            "ontology____bounding_box_3d",
            "scene_index",
    ...
    def test_dataset(self):
        ...
        assert set(dataset.ontology_table.keys()) == {
            "bounding_box_2d",
            "bounding_box_3d",
        }
