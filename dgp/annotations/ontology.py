    ...
    def __init__(self, ontology_pb2):
        ...
        if isinstance(self._ontology, OntologyV1Pb2):
            ...
            self._colormap = OrderedDict(
                sorted(
                    [
                        (_id, (_color.r, _color.g, _color.b))
                        for _id, _color in self._ontology.colormap.items()
                    ]
                )
            )
                ...
                sorted(
                    [
                        (
                            ontology_item.id,
                            (
                                ontology_item.color.r,
                                ontology_item.color.g,
                                ontology_item.color.b,
                            ),
                        )
                        for ontology_item in self._ontology.items
                    ]
            ...
            self._isthing = OrderedDict(
                sorted(
                    [
                        (ontology_item.id, ontology_item.isthing)
                        for ontology_item in self._ontology.items
                    ]
                )
            )
            ...
            raise TypeError(
                "Unexpected type {}, expected OntologyV1 or OntologyV2".format(
                    type(self._ontology)
                )
            )
            ...
            items=[
                OntologyItem(
                    ...
                    color=OntologyItem.Color(
                        r=self._colormap[class_id][0],
                        g=self._colormap[class_id][1],
                        b=self._colormap[class_id][2],
                    ),
...
class BoundingBoxOntology(Ontology):
    ...
    def __init__(self, ontology_pb2):
        ...
        self._thing_class_ids = [
            class_id for class_id, isthing in self._isthing.items() if isthing
        ]
        ...
        self._class_id_to_contiguous_id = OrderedDict(
            (class_id, contiguous_id + 1)
            for contiguous_id, class_id in enumerate(self._thing_class_ids)
        )

        # Contiguous ID map back to original class ID
        self._contiguous_id_to_class_id = OrderedDict(
            (contiguous_id, class_id)
        ...
        self._contiguous_id_to_name = OrderedDict(
            (contiguous_id, self._id_to_name[class_id])
            for contiguous_id, class_id in self._contiguous_id_to_class_id.items()
        )
        self._name_to_contiguous_id = OrderedDict(
            (name, contiguous_id)
            for contiguous_id, name in self._contiguous_id_to_name.items()
        )
...
class SemanticSegmentationOntology(Ontology):
    ...
    def __init__(self, ontology_pb2):
        ...
        self._class_id_to_contiguous_id = OrderedDict(
            (class_id, contiguous_id)
            for contiguous_id, class_id in enumerate(self._class_ids)
        )
        ...
        self._label_lookup = (
            np.ones(max(self.class_ids) + 1, dtype=np.uint8) * self.VOID_ID
        )
        for class_id, contiguous_id in self._class_id_to_contiguous_id.items():
