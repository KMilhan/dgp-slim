...
class PanopticSegmentation2DAnnotation(Annotation):
    ...
    def __init__(
            ...
            panoptic_image,
            index_to_label,
            panoptic_image_dtype=DEFAULT_PANOPTIC_IMAGE_DTYPE,
    ):
        if panoptic_image_dtype not in (np.uint16, np.uint32):
            raise ValueError(
                "'panoptic_image_dtype' must be either np.uint16 or np.uint32."
            )
        ...
        if (
                not isinstance(panoptic_image, np.ndarray)
                or not panoptic_image.dtype == panoptic_image_dtype
                or len(panoptic_image.shape) != 2
        ):
            raise ValueError(
                "`panoptic_image` needs to be a single-channel uint16 numpy array"
            )
        ...
        for class_name, labels in self.index_to_label.items():
            if isinstance(labels, list):
                for label in labels:
                    ...
                    if instance_id < 0:
                        raise ValueError(
                            "`index` field of a thing class is expected to be non-negative"
                        )
    ...
    def class_names(self):
        ...
        """
        return [
            self.ontology.contiguous_id_to_name[class_id] for class_id in self.class_ids
        ]
    ...
    def from_masklist(
        ...
        if not masklist:
            panoptic_image = (
                    np.ones(mask_shape, panoptic_image_dtype) * ontology.VOID_ID
            )
        else:
            panoptic_image = (
                    np.ones_like(masklist[0].bitmask, panoptic_image_dtype)
                    * ontology.VOID_ID
            )
        index_to_label = {class_name: [] for class_name in ontology.class_names}
        for instance_mask in masklist:
            assert np.unique(panoptic_image[instance_mask.bitmask]) == [
                ontology.VOID_ID
            ], "No overlapping between instance masks is allowed."
            panoptic_image[instance_mask.bitmask] = instance_mask.instance_id
            index_to_label[
                ontology.contiguous_id_to_name[instance_mask.class_id]
            ].append(
                {
                    "index": instance_mask.instance_id,
                    "attributes": instance_mask.attributes,
                }
            )
    ...
    def hexdigest(self):
        ...
        return generate_uid_from_instance_segmentation_2d_annotation(
            self.panoptic_image
        )
