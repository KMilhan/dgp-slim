...
class Compose:
    ...
    def __init__(self, transforms):
        if not all([isinstance(t, BaseTransform) for t in transforms]):
            raise TypeError(
                "All transforms used in Compose should inherit from `BaseTransform`"
            )
...
class BaseTransform:
    ...
    def __call__(self, data):
        if not isinstance(data, OrderedDict) and not (
            ...
            and isinstance(data[0][0], OrderedDict)
        ):
            raise TypeError(
                "`BaseTransform` expects input of type `OrderedDict` or list of list of `OrderedDict`."
            )
...
class OntologyMapper(BaseTransform):
    ...
    def __init__(
        self, original_ontology_table, lookup_table, remapped_ontology_table=None
    ):
        for annotation_key in lookup_table:
            if annotation_key not in self.SUPPORTED_ANNOTATION_TYPES:
                raise ValueError(
                    f"annotation_key {annotation_key} not supported for remapping yet, we accept PRs"
                )
            if annotation_key not in original_ontology_table:
                raise ValueError(
                    f"annotation_key {annotation_key} needs to be present in `ontology_table`"
                )
        ...
        for annotation_key, lookup in self.lookup_table.items():
            assert all(
                [
                    class_name in original_ontology_table[annotation_key].class_names
                    for class_name in lookup.keys()
                ]
            ), "All keys in `lookup` need to be valid class names in specified `ontology`"
            if (
                remapped_ontology_table is not None
                and annotation_key in remapped_ontology_table
            ):
                self.remapped_ontology_table[annotation_key] = remapped_ontology_table[
                    annotation_key
                ]
            else:
                ...
                self.remapped_ontology_table[annotation_key] = ONTOLOGY_REGISTRY[
                    annotation_key
                ](remapped_ontology_pb2)
            assert all(
                [
                    class_name
                    in self.remapped_ontology_table[annotation_key].class_names
                    for class_name in lookup.values()
                ]
    ...
    def transform_datum(self, datum):
        ...
        """
        if not all(
            [annotation_key in datum for annotation_key in self.remapped_ontology_table]
        ):
            raise ValueError(
                "The data you are trying to remap does not have all annotations it expects"
            )
        ...
        for annotation_key, remapped_ontology in self.remapped_ontology_table.items():
            ...
            if annotation_key in ("bounding_box_2d", "bounding_box_3d"):
                datum[annotation_key] = remap_bounding_box_annotations(
                    datum[annotation_key],
                    lookup_table,
                    original_ontology,
                    remapped_ontology,
                )
